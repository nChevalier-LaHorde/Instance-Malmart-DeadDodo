...
H3_DECLARE_COMPONENT_PROPERTY_ACCESSORS_RW_EX(CharacterComponent, int, stateMenu);


H3_CAPI_END_BLOCK

#define CHARACTERCOMPONENT_CREATE(H3, STATEMENU)							\
	(SH3Component) {													\
		.Terminate          = CharacterComponent_Terminate,				\
		.Update             = CharacterComponent_Update,					\
		.isInitialized      = false,									\
		.componentType      = CHARACTERCOMPONENT_TYPEID,					\
		.properties         = CharacterComponent_CreateProperties(H3, STATEMENU)		\
	}
